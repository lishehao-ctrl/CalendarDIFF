...
from app.modules.auth.service import _hash_password, _normalize_notify_email, _normalize_timezone_name
...
def ensure_env_admin_user(
    ...
    *,
    ...
    db.refresh(user)
    logger.info(
        "bootstrap admin user %s notify_email=%s user_id=%s",
...
def bootstrap_env_admin_user(_: FastAPI) -> None:
    ...
    notify_email = (settings.bootstrap_admin_notify_email or "").strip()
    password = settings.bootstrap_admin_password or ""
    if not notify_email or not password:
        return
