...
def parse_calendar_changed_component_with_llm(
    *,
    ...
    cached_records = load_cached_calendar_component_records(
        db=db,
        source_id=context.source_id,
        fingerprint=component.fingerprint,
    )
    if cached_records is not None:
        increment_parse_metric_counter(redis_client, metric_name="calendar_component_parse_cache_hit")
...
def _parse_single_gmail_message(
    *,
    ...
    with session_factory() as db:
        cached_records = load_cached_gmail_parse_records(
            db=db,
            source_id=source_id,
            payload_item=payload_item,
        )
        if cached_records is not None:
            increment_parse_metric_counter(redis_client, metric_name="gmail_parse_cache_hit")
            return cached_records
        ...
        try:
            ...
            return []
        records = attach_parser_metadata(records=parser_output.records, parser_output=parser_output)
        store_cached_gmail_parse_records(
            db=db,
            source_id=source_id,
            payload_item=payload_item,
