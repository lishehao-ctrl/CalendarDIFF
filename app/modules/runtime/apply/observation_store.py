...
def normalize_observation_payload(payload: dict) -> dict:
    if not isinstance(payload, dict):
        raise RuntimeError("runtime_apply_payload_invalid: observation payload must be object")
    if "semantic_event_draft" in payload or "enrichment" in payload:
        raise RuntimeError("runtime_apply_payload_invalid: observation payload must use runtime semantic_event envelope")
    ...
    for key in _RUNTIME_OBSERVATION_KEYS:
        ...
        if not isinstance(value, dict):
            raise RuntimeError(f"runtime_apply_payload_invalid: observation payload missing {key}")
