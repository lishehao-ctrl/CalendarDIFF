    ...
    for row in rows:
        ...
        if attempt < max_attempts:
            ...
            apply_retry_transition(
                ...
                sync_substage="queued",
                sync_progress=build_sync_progress_payload(
                    phase="queued",
                    label="Queued to retry",
                    detail="The sync hit a claim timeout and was requeued for another connector attempt.",
                ),
    ...
    for row in rows:
        ...
        if sync_request is not None:
            set_sync_runtime_state(
                ...
                substage="connector_claimed",
                progress=build_sync_progress_payload(
                    phase="connector_fetch",
                    label="Connector fetch starting",
                    detail="The source connector has claimed this sync and is preparing provider fetch work.",
                ),
                error_code=None,
                error_message=None,
                when=now,
