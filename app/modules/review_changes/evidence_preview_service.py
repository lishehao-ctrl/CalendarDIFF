...
from app.modules.common.payload_schemas import FrozenReviewEvidence
...
def preview_review_change_evidence(
    ...
    *,
    ...
    if not isinstance(evidence, dict):
        raise ReviewChangeEvidenceNotFoundError("Evidence preview unavailable")
    try:
        normalized = FrozenReviewEvidence.model_validate(evidence)
    except Exception as exc:  # pragma: no cover - defensive malformed frozen evidence path
        raise ReviewChangeEvidenceReadError("Evidence preview is malformed") from exc
