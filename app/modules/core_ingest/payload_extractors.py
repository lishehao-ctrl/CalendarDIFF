...
from app.modules.core_ingest.source_facts_coercion import coerce_text, parse_optional_iso_datetime
from app.modules.core_ingest.semantic_event_service import (
...
def extract_source_facts_from_calendar_payload(
    *,
    ...
    if raw is None:
        raise RuntimeError("core_ingest_payload_invalid: calendar payload missing source_facts")
    ...
    if not isinstance(source_title, str) or not source_title.strip():
        raise RuntimeError("core_ingest_payload_invalid: calendar payload missing source_facts.source_title")
    if not isinstance(source_dtstart, str) or not source_dtstart.strip():
        raise RuntimeError("core_ingest_payload_invalid: calendar payload missing source_facts.source_dtstart_utc")
    if not isinstance(source_dtend, str) or not source_dtend.strip():
        raise RuntimeError("core_ingest_payload_invalid: calendar payload missing source_facts.source_dtend_utc")
...
def extract_source_facts_from_gmail_payload(*, payload: dict) -> dict:
    raw = payload.get("source_facts") if isinstance(payload.get("source_facts"), dict) else None
    if raw is None:
        raise RuntimeError("core_ingest_payload_invalid: gmail payload missing source_facts")
    ...
    source_title = raw.get("source_title")
    if not isinstance(source_title, str) or not source_title.strip():
        raise RuntimeError("core_ingest_payload_invalid: gmail payload missing source_facts.source_title")
    external_event_id = raw.get("external_event_id")
    if not isinstance(external_event_id, str) or not external_event_id.strip():
        raise RuntimeError("core_ingest_payload_invalid: gmail payload missing source_facts.external_event_id")
    source_facts = SourceFacts.model_validate(
        {
