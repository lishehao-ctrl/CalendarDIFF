...
function MetricCard({ label, value, detail }: { label: string; value: string; detail: string }) {
  return (
    <Card className="p-5">
      <p className="text-xs uppercase tracking-[0.2em] text-[#6d7885]">{label}</p>
      <p className="mt-3 text-3xl font-semibold">{value}</p>
      <p className="mt-2 text-sm text-[#596270]">{detail}</p>
    </Card>
  );
}
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          label="Candidates"
          value={String(summary.data?.link_candidates_pending ?? 0)}
          detail="Source events that still need a yes or no."
        />
        <MetricCard
          label="Active links"
          value={String(links.data?.length ?? 0)}
          detail="Current source-to-entity bindings in this review window."
        />
        <MetricCard
          label="Blocks"
          value={String(blocks.data?.length ?? 0)}
          detail="Known bad pairings that should not auto-link again."
        />
        <MetricCard
          label="Snapshot"
          value={formatDateTime(summary.data?.generated_at, "Just now")}
          detail="Latest link-review summary generated by the backend."
        />
      </div>
        ...
        <Card className="p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="max-w-2xl">
              ...
              <h3 className="mt-3 text-xl font-semibold">Approve or reject candidate matches</h3>
              <p className="mt-2 text-sm text-[#596270]">
                When a source event looks like an existing deadline, it lands here for a quick human decision before the system trusts the match.
              </p>
            </div>
          ...
          <Card className="p-5">
            <div className="flex items-start justify-between gap-4">
              <div className="max-w-2xl">
                ...
                <p className="mt-2 text-sm text-[#596270]">
                  These links are currently trusted. Unlinking one will also create a block so the same bad match does not immediately reappear.
                </p>
              </div>
          ...
          <Card className="p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h3 className="mt-3 text-xl font-semibold">Relink a source event</h3>
                <p className="mt-2 text-sm text-[#596270]">Use this when the correct entity is already known and you want to repair the binding directly.</p>
              </div>
          ...
          <Card className="p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h3 className="mt-3 text-xl font-semibold">Do-not-link blocks</h3>
                <p className="mt-2 text-sm text-[#596270]">Blocks are created by human review to stop the same bad pairing from being suggested again.</p>
              </div>
