...
const items = [
  ...
  { href: "/families/add", label: "Add Family" },
] as const;

export function FamilySubnav({ basePath = "" }: { basePath?: string }) {
  ...
  return (
    <div className="inline-flex flex-wrap items-center gap-1 rounded-[1rem] border border-line/80 bg-white/72 p-1 shadow-[var(--shadow-panel)]">
      {items.map((item) => {
        ...
        const active = pathname === href || (item.href === "/families" && pathname?.startsWith(`${href}/`));
        return (
          <Link
            key={item.href}
            href={href}
