...
CalendarDIFF 的 LLM 网关会按 `INGESTION_LLM_API_MODE` 选择 `/responses` 或 `/chat/completions`。在 `docker compose` 下，`INGESTION_LLM_MODEL`、`INGESTION_LLM_BASE_URL`、`INGESTION_LLM_API_KEY` 都是必填；`INGESTION_LLM_EXTRA_BODY_JSON` 可用于透传 provider 私有参数，例如 `{\"enable_thinking\":false}`，超时和重试也可以通过 `INGESTION_LLM_TIMEOUT_SECONDS`、`INGESTION_LLM_MAX_RETRIES`、`INGESTION_LLM_MAX_INPUT_CHARS` 调整。
