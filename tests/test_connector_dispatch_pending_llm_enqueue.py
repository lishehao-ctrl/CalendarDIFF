...
def test_dispatch_pending_enqueue_success_marks_queued(db_session: Session, monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.modules.ingestion.connector_dispatch.get_redis_client", lambda: object())
    monkeypatch.setattr("app.modules.ingestion.connector_dispatch.ensure_stream_group", lambda *_args, **_kwargs: None)

    def _enqueue(**kwargs):  # noqa: ANN003
        captured.update(kwargs)
        return "msg-id-1"

    monkeypatch.setattr("app.modules.ingestion.connector_dispatch.enqueue_llm_task", _enqueue)
...
def test_dispatch_pending_enqueue_failure_keeps_pending_before_threshold(db_session: Session, monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.modules.ingestion.connector_dispatch.ensure_stream_group", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(
        "app.modules.ingestion.connector_dispatch.enqueue_llm_task",
