...
def test_source_deadlines_endpoint_returns_grouped_output(client, monkeypatch) -> None:
    ...
    create_response = client.post(
        "/v1/sources/ics",
        headers=headers,
        json={"name": "School Calendar", "url": "https://example.com/calendar.ics"},
    )
    assert create_response.status_code == 201
    source_id = create_response.json()["id"]

    def fake_fetch(self, url: str, source_id: int):
        return FetchResult(
            content=SAMPLE_ICS,
            etag=None,
    ...
    monkeypatch.setattr("app.modules.sources.service.ICSClient.fetch", fake_fetch)

    response = client.get(f"/v1/sources/{source_id}/deadlines", headers=headers)
    assert response.status_code == 200

    payload = response.json()
    assert payload["source_id"] == source_id
    assert payload["source_name"] == "School Calendar"
