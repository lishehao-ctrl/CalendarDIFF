...
def test_gmail_directive_partial_term_out_of_scope_creates_partial_changes_and_isolation(db_session) -> None:
    ...
    records = [
        _directive_record(
            message_id="msg-dir-partial",
            external_event_id="msg-dir-partial#directive-seg-0",
            selector={
                "course_dept": "CSE",
                "course_number": 8,
                "course_suffix": "A",
    ...
    _seed_ingest_result(db_session, source=source, request_id="directive-apply-partial", records=records)
    result = apply_ingest_result_idempotent(db_session, request_id="directive-apply-partial")
    assert result["changes_created"] == 1
    ...
    assert len(rows) == 1
    assert rows[0].entity_uid == "ent-partial-in"
    assert rows[0].after_semantic_json["due_date"] == "2026-03-20"

    unresolved = db_session.scalar(
        select(IngestUnresolvedRecord).where(
            IngestUnresolvedRecord.user_id == user.id,
            IngestUnresolvedRecord.source_id == source.id,
            IngestUnresolvedRecord.external_event_id == "msg-dir-partial#directive-seg-0#directive:term_out_of_scope",
            IngestUnresolvedRecord.is_active.is_(True),
        )
    )
    assert unresolved is not None
