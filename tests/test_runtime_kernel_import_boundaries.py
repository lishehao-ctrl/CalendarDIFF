...
RUNTIME_KERNEL_ROOT = REPO_ROOT / "app" / "modules" / "runtime_kernel"
    ...
    for py_path in RUNTIME_KERNEL_ROOT.rglob("*.py"):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom):
                ...
                if module_name.startswith("app.modules.ingestion") or module_name.startswith("app.modules.llm_runtime"):
                    violations.append(f"{py_path.relative_to(REPO_ROOT)}:{node.lineno} imports {module_name}")
            elif isinstance(node, ast.Import):
                for alias in node.names:
                    name = alias.name
                    if name.startswith("app.modules.ingestion") or name.startswith("app.modules.llm_runtime"):
