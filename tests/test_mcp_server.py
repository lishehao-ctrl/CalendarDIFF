...
from services.mcp_server.main import (
    ...
    create_label_learning_commit_proposal_impl,
...
def _create_editable_change(db_session, *, user: User, source, family: CourseWorkItemLabelFamily) -> Change:
    change = Change(
        user_id=user.id,
        entity_uid="mcp-proposal-edit",
    ...
    db_session.add(
        ChangeSourceRef(
            ...
            external_event_id="evt-mcp-proposal-edit",
            confidence=0.95,
        )
    )
    db_session.commit()
...
def test_mcp_server_registers_expected_tools_and_resources() -> None:
    ...
    assert "create_change_edit_commit_proposal" in tool_names
...
def test_mcp_impl_round_trip_uses_existing_agent_layers(db_session) -> None:
    ...
    workspace = get_workspace_context_impl(email=user.email)
    change_context = get_change_context_impl(change_id=change.id, email=user.email)
    source_context = get_source_context_impl(source_id=source.id, email=user.email)
    family_context = get_family_context_impl(family_id=family.id, email=user.email)
    proposal = create_change_decision_proposal_impl(change_id=change.id, email=user.email)
    fetched_proposal = get_proposal_impl(proposal_id=proposal["proposal_id"], email=user.email)
    proposals = list_proposals_impl(email=user.email)
    ticket = create_approval_ticket_impl(proposal_id=proposal["proposal_id"], email=user.email)
    tickets = list_approval_tickets_impl(email=user.email)
    activity = get_recent_agent_activity_impl(email=user.email)
...
def test_mcp_change_edit_commit_proposal_impl_is_executable(db_session) -> None:
    user = _create_user(db_session, email="mcp-change-edit@example.com")
    source = _create_source(db_session, user=user, provider="ics")
    family = _create_family(db_session, user_id=user.id, course_display="CSE 170 WI26", canonical_label="Quiz")
    change = _create_editable_change(db_session, user=user, source=source, family=family)

    proposal = create_change_edit_commit_proposal_impl(
        change_id=change.id,
        patch={"due_date": "2026-03-24", "event_name": "Quiz 1 (edited)"},
        email=user.email,
    )

    assert proposal["proposal_type"] == "proposal_edit_commit"
    assert proposal["suggested_payload"]["kind"] == "proposal_edit_commit"
    assert proposal["can_create_ticket"] is True
    assert proposal["origin_label"] == "create_change_edit_commit_proposal"


def test_mcp_family_relink_commit_proposal_impl_is_executable(db_session) -> None:
    user = _create_user(db_session, email="mcp-family-commit@example.com")
    source_family = _create_family(db_session, user_id=user.id, course_display="CSE 170 WI26", canonical_label="Quiz")
    target_family = _create_family(db_session, user_id=user.id, course_display="CSE 170 WI26", canonical_label="Project")
    raw_type = CourseWorkItemRawType(
        family_id=source_family.id,
    ...
    proposal = create_family_relink_commit_proposal_impl(
        raw_type_id=raw_type.id,
        family_id=target_family.id,
        email=user.email,
    )
    ...
    assert proposal["proposal_type"] == "family_relink_preview"
    assert proposal["suggested_payload"]["kind"] == "family_relink_commit"
    assert proposal["can_create_ticket"] is True
    assert proposal["origin_label"] == "create_family_relink_commit_proposal"


def test_mcp_label_learning_commit_proposal_impl_is_executable(db_session) -> None:
    user = _create_user(db_session, email="mcp-label-learning@example.com")
    source = _create_source(db_session, user=user, provider="ics")
    family = _create_family(db_session, user_id=user.id, course_display="CSE 100 WI26", canonical_label="Homework")
    change = Change(
        user_id=user.id,
        entity_uid="mcp-label-learning",
        change_origin=ChangeOrigin.INGEST_PROPOSAL,
    ...
    db_session.add(
        ChangeSourceRef(
            ...
            source_kind=source.source_kind,
            provider=source.provider,
            external_event_id="evt-mcp-label-learning",
            confidence=0.95,
        )
    )
    ...
    db_session.add(
        SourceEventObservation(
            ...
            entity_uid="mcp-label-learning",
            event_payload={
                "source_facts": {"external_event_id": "evt-mcp-label-learning"},
                "semantic_event": {
                    "uid": "mcp-label-learning",
                    "course_dept": "CSE",
                    "course_number": 100,
                ...
                "link_signals": {},
                "kind_resolution": {"status": "unresolved", "reason_code": "missing_course_identity"},
            },
            event_hash="2" * 64,
    ...
    proposal = create_label_learning_commit_proposal_impl(
        change_id=change.id,
        family_id=family.id,
        email=user.email,
    )

    assert proposal["proposal_type"] == "label_learning_commit"
    assert proposal["suggested_payload"]["kind"] == "label_learning_add_alias_commit"
    assert proposal["can_create_ticket"] is True
    assert proposal["origin_label"] == "create_label_learning_commit_proposal"
