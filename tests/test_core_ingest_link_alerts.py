from __future__ import annotations

import json
from datetime import datetime, timedelta, timezone

from sqlalchemy import func, select
from sqlalchemy.orm import Session

from app.core.security import encrypt_secret
from app.db.models.ingestion import ConnectorResultStatus, IngestResult
from app.db.models.input import IngestTriggerType, InputSource, InputSourceConfig, InputSourceCursor, InputSourceSecret, SourceKind, SyncRequest, SyncRequestStatus
from app.db.models.review import Change, EventEntityLink, EventLinkAlert, EventLinkAlertResolution, EventLinkAlertStatus, EventLinkCandidate, EventLinkCandidateStatus, EventLinkOrigin, Input, InputType, ReviewStatus
from app.db.models.shared import User
from app.modules.core_ingest.apply_service import apply_ingest_result_idempotent
from app.modules.review_changes.change_decision_service import decide_review_change
from app.modules.review_links.alerts_event_consumer import run_review_link_alert_events_tick
from tests.support.payload_builders import (
    build_calendar_payload,
    build_course_parse,
    build_event_parts,
    build_gmail_payload,
    build_link_signals,
)


def _create_sources(db_session: Session) -> tuple[InputSource, InputSource]:
    now = datetime.now(timezone.utc)
    user = User(
        email="link-alert-owner@example.com",
        notify_email="link-alert-owner@example.com",
        onboarding_completed_at=now,
    )
    db_session.add(user)
    db_session.flush()

    calendar_source = InputSource(
        user_id=user.id,
        source_kind=SourceKind.CALENDAR,
        provider="ics",
        source_key="link-alert-calendar",
        display_name="Link Alert Calendar",
        is_active=True,
        poll_interval_seconds=900,
        next_poll_at=now,
    )
    gmail_source = InputSource(
        user_id=user.id,
        source_kind=SourceKind.EMAIL,
        provider="gmail",
        source_key="link-alert-gmail",
        display_name="Link Alert Gmail",
        is_active=True,
        poll_interval_seconds=900,
        next_poll_at=now,
    )
    db_session.add_all([calendar_source, gmail_source])
    db_session.flush()

    db_session.add(InputSourceConfig(source_id=calendar_source.id, schema_version=1, config_json={}))
    db_session.add(
        InputSourceSecret(
            source_id=calendar_source.id,
            encrypted_payload=encrypt_secret(json.dumps({"url": "https://example.com/link-alert.ics"})),
        )
    )
    db_session.add(InputSourceCursor(source_id=calendar_source.id, version=1, cursor_json={}))

    db_session.add(InputSourceConfig(source_id=gmail_source.id, schema_version=1, config_json={"label_id": "INBOX"}))
    db_session.add(
        InputSourceSecret(
            source_id=gmail_source.id,
            encrypted_payload=encrypt_secret(json.dumps({"access_token": "token", "account_email": "user@example.edu"})),
        )
    )
    db_session.add(InputSourceCursor(source_id=gmail_source.id, version=1, cursor_json={"history_id": "101"}))

    db_session.commit()
    db_session.refresh(calendar_source)
    db_session.refresh(gmail_source)
    return calendar_source, gmail_source


def _seed_result(db_session: Session, *, source: InputSource, request_id: str, records: list[dict]) -> None:
    db_session.add(
        SyncRequest(
            request_id=request_id,
            source_id=source.id,
            trigger_type=IngestTriggerType.MANUAL,
            status=SyncRequestStatus.RUNNING,
            idempotency_key=f"idemp:{request_id}",
            metadata_json={"kind": "test"},
        )
    )
    db_session.add(
        IngestResult(
            request_id=request_id,
            source_id=source.id,
            provider=source.provider,
            status=ConnectorResultStatus.CHANGED,
            cursor_patch={},
            records=records,
            fetched_at=datetime.now(timezone.utc),
            error_code=None,
            error_message=None,
        )
    )
    db_session.commit()


def _canonical_input_id(db_session: Session, *, user_id: int) -> int:
    row = db_session.scalar(
        select(Input).where(
            Input.user_id == user_id,
            Input.type == InputType.ICS,
            Input.identity_key == f"canonical:user:{user_id}",
        )
    )
    assert row is not None
    return row.id


def _drain_link_alert_events(db_session: Session) -> int:
    return run_review_link_alert_events_tick(db_session, batch_limit=200)


def _approve_all_pending_changes(db_session: Session, *, user_id: int, canonical_input_id: int) -> list[Change]:
    pending_rows = db_session.scalars(
        select(Change)
        .where(Change.input_id == canonical_input_id, Change.review_status == ReviewStatus.PENDING)
        .order_by(Change.id.asc())
    ).all()
    assert pending_rows
    for row in pending_rows:
        decide_review_change(
            db_session,
            user_id=user_id,
            change_id=row.id,
            decision="approve",
            note="approve seed",
        )
    return pending_rows


def _calendar_exam_record(
    *,
    external_event_id: str,
    title: str,
    start_at: datetime,
    end_at: datetime,
    dept: str = "CSE",
    number: int = 151,
    suffix: str | None = None,
    index: int = 1,
) -> dict:
    return {
        "record_type": "calendar.event.extracted",
        "payload": build_calendar_payload(
            external_event_id=external_event_id,
            title=title,
            start_at=start_at,
            end_at=end_at,
            course_parse=build_course_parse(
                dept=dept,
                number=number,
                suffix=suffix,
                quarter="WI",
                year2=26,
                confidence=0.95,
                evidence=f"{dept} {number}{suffix or ''} WI26",
            ),
            event_parts=build_event_parts(
                type="exam",
                index=index,
                confidence=0.95,
                evidence=f"exam {index}",
            ),
            link_signals=build_link_signals(
                keywords=["exam"],
                exam_sequence=index,
            ),
        ),
    }


def _gmail_exam_record(
    *,
    message_id: str,
    title: str,
    due_at: datetime | None,
    time_anchor_confidence: float,
    dept: str = "CSE",
    number: int = 151,
    suffix: str | None = None,
    index: int | None = 1,
) -> dict:
    return {
        "record_type": "gmail.message.extracted",
        "payload": build_gmail_payload(
            message_id=message_id,
            title=title,
            due_at=due_at,
            time_anchor_confidence=time_anchor_confidence,
            from_header="Prof Alice <alice@ucsd.edu>",
            course_parse=build_course_parse(
                dept=dept,
                number=number,
                suffix=suffix,
                confidence=0.92,
                evidence=f"{dept}{number}{suffix or ''}",
            ),
            event_parts=build_event_parts(
                type="exam",
                index=index,
                confidence=0.9,
                evidence=f"exam {index}" if index is not None else "exam",
            ),
            link_signals=build_link_signals(
                keywords=["exam"],
                exam_sequence=index,
                instructor_hint="Prof Alice",
            ),
        ),
    }


def _seed_auto_link_without_pending(
    db_session: Session,
) -> tuple[InputSource, InputSource, int, str]:
    calendar_source, gmail_source = _create_sources(db_session)
    due_at = datetime(2026, 3, 15, 20, 0, tzinfo=timezone.utc)
    end_at = due_at + timedelta(hours=1)

    _seed_result(
        db_session,
        source=calendar_source,
        request_id="alert-cal-seed",
        records=[
            _calendar_exam_record(
                external_event_id="ics-alert-exam-1",
                title="CSE 151 exam 1",
                start_at=due_at,
                end_at=end_at,
            )
        ],
    )
    first_apply = apply_ingest_result_idempotent(db_session, request_id="alert-cal-seed")
    assert first_apply["changes_created"] == 1

    canonical_input_id = _canonical_input_id(db_session, user_id=calendar_source.user_id)
    approved = _approve_all_pending_changes(
        db_session,
        user_id=calendar_source.user_id,
        canonical_input_id=canonical_input_id,
    )
    event_uid = approved[-1].event_uid

    _seed_result(
        db_session,
        source=gmail_source,
        request_id="alert-gmail-seed",
        records=[
            _gmail_exam_record(
                message_id="gmail-alert-exam-1",
                title="CSE151 exam 1",
                due_at=due_at,
                time_anchor_confidence=0.95,
            )
        ],
    )
    second_apply = apply_ingest_result_idempotent(db_session, request_id="alert-gmail-seed")
    assert second_apply["changes_created"] == 0
    _drain_link_alert_events(db_session)
    return calendar_source, gmail_source, canonical_input_id, event_uid


def test_auto_link_without_canonical_pending_creates_link_alert(db_session: Session) -> None:
    calendar_source, gmail_source, canonical_input_id, event_uid = _seed_auto_link_without_pending(db_session)

    link_row = db_session.scalar(
        select(EventEntityLink).where(
            EventEntityLink.user_id == calendar_source.user_id,
            EventEntityLink.source_id == gmail_source.id,
            EventEntityLink.external_event_id == "gmail-alert-exam-1",
        )
    )
    assert link_row is not None
    assert link_row.link_origin == EventLinkOrigin.AUTO
    assert link_row.entity_uid == event_uid

    alert_row = db_session.scalar(
        select(EventLinkAlert).where(
            EventLinkAlert.user_id == calendar_source.user_id,
            EventLinkAlert.source_id == gmail_source.id,
            EventLinkAlert.external_event_id == "gmail-alert-exam-1",
            EventLinkAlert.entity_uid == event_uid,
        )
    )
    assert alert_row is not None
    assert alert_row.status == EventLinkAlertStatus.PENDING
    assert alert_row.resolution_code is None
    assert alert_row.reason_code.value == "auto_link_without_canonical_change"
    assert isinstance(alert_row.evidence_snapshot_json, dict)

    pending_count = db_session.scalar(
        select(func.count(Change.id)).where(
            Change.input_id == canonical_input_id,
            Change.review_status == ReviewStatus.PENDING,
        )
    )
    assert int(pending_count or 0) == 0


def test_link_alert_resolved_when_candidate_opened_for_same_pair(db_session: Session) -> None:
    calendar_source, gmail_source, _, event_uid = _seed_auto_link_without_pending(db_session)

    _seed_result(
        db_session,
        source=gmail_source,
        request_id="alert-gmail-candidate",
        records=[
            _gmail_exam_record(
                message_id="gmail-alert-exam-1",
                title="CSE151 exam 1 (missing due)",
                due_at=None,
                time_anchor_confidence=0.1,
            )
        ],
    )
    candidate_apply = apply_ingest_result_idempotent(db_session, request_id="alert-gmail-candidate")
    assert candidate_apply["changes_created"] == 0
    _drain_link_alert_events(db_session)

    candidate_row = db_session.scalar(
        select(EventLinkCandidate).where(
            EventLinkCandidate.user_id == calendar_source.user_id,
            EventLinkCandidate.source_id == gmail_source.id,
            EventLinkCandidate.external_event_id == "gmail-alert-exam-1",
            EventLinkCandidate.status == EventLinkCandidateStatus.PENDING,
        )
    )
    assert candidate_row is not None

    alert_row = db_session.scalar(
        select(EventLinkAlert).where(
            EventLinkAlert.user_id == calendar_source.user_id,
            EventLinkAlert.source_id == gmail_source.id,
            EventLinkAlert.external_event_id == "gmail-alert-exam-1",
            EventLinkAlert.entity_uid == event_uid,
        )
    )
    assert alert_row is not None
    assert alert_row.status == EventLinkAlertStatus.RESOLVED
    assert alert_row.resolution_code == EventLinkAlertResolution.CANDIDATE_OPENED


def test_link_alert_resolved_when_canonical_pending_created(db_session: Session) -> None:
    calendar_source, gmail_source, canonical_input_id, event_uid = _seed_auto_link_without_pending(db_session)
    original_due = datetime(2026, 3, 15, 20, 0, tzinfo=timezone.utc)
    shifted_due = datetime(2026, 3, 16, 20, 0, tzinfo=timezone.utc)

    _seed_result(
        db_session,
        source=calendar_source,
        request_id="alert-cal-shift",
        records=[
            _calendar_exam_record(
                external_event_id="ics-alert-exam-1",
                title="CSE 151 exam 1 shifted",
                start_at=shifted_due,
                end_at=shifted_due + timedelta(hours=1),
            )
        ],
    )
    update_apply = apply_ingest_result_idempotent(db_session, request_id="alert-cal-shift")
    assert update_apply["changes_created"] == 1
    _drain_link_alert_events(db_session)

    pending_changes = db_session.scalars(
        select(Change).where(
            Change.input_id == canonical_input_id,
            Change.event_uid == event_uid,
            Change.review_status == ReviewStatus.PENDING,
        )
    ).all()
    assert len(pending_changes) == 1
    after_json = pending_changes[0].after_json if isinstance(pending_changes[0].after_json, dict) else {}
    assert str(after_json.get("start_at_utc")).startswith(shifted_due.isoformat()[:19])
    assert not str(after_json.get("start_at_utc")).startswith(original_due.isoformat()[:19])

    alert_row = db_session.scalar(
        select(EventLinkAlert).where(
            EventLinkAlert.user_id == calendar_source.user_id,
            EventLinkAlert.source_id == gmail_source.id,
            EventLinkAlert.external_event_id == "gmail-alert-exam-1",
            EventLinkAlert.entity_uid == event_uid,
        )
    )
    assert alert_row is not None
    assert alert_row.status == EventLinkAlertStatus.RESOLVED
    assert alert_row.resolution_code == EventLinkAlertResolution.CANONICAL_PENDING_CREATED
