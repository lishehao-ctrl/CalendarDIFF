...
def test_course_work_item_family_crud(input_client, db_session, authenticate_client) -> None:
    ...
    list_response = input_client.get("/review/course-work-item-families", headers={"X-API-Key": "test-api-key"})
    ...
    courses_response = input_client.get("/review/course-work-item-families/courses", headers={"X-API-Key": "test-api-key"})
    assert courses_response.status_code == 200
    assert courses_response.json()["courses"][0]["course_display"] == "CSE 100 WI26"

    update_response = input_client.patch(
        f"/review/course-work-item-families/{created['id']}",
        headers={"X-API-Key": "test-api-key"},
    ...
    status_response = input_client.get("/review/course-work-item-families/status", headers={"X-API-Key": "test-api-key"})
    ...
    after_delete_attempt = input_client.get("/review/course-work-item-families", headers={"X-API-Key": "test-api-key"})
