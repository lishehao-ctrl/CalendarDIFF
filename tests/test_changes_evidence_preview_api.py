...
def test_preview_change_evidence_before_and_after_success(client, initialized_user, db_session: Session, monkeypatch, tmp_path) -> None:
    ...
    before_preview = client.get(
        f"/v1/inputs/{input_id}/changes/{change_id}/evidence/before/preview",
        headers=headers,
    )
    ...
    after_preview = client.get(
        f"/v1/inputs/{input_id}/changes/{change_id}/evidence/after/preview",
        headers=headers,
    )
...
def test_preview_change_evidence_before_missing_returns_404(client, db_session: Session) -> None:
    ...
    input_row = Input(
        user_id=user.id,
        type=InputType.ICS,
        name="Preview Missing Before",
        normalized_name="preview missing before",
    ...
    headers = {"X-API-Key": "test-api-key"}
    response = client.get(f"/v1/inputs/{input_row.id}/changes/{change.id}/evidence/before/preview", headers=headers)
...
def test_preview_change_evidence_path_traversal_returns_404(client, db_session: Session, monkeypatch, tmp_path) -> None:
    ...
    input_row = Input(
        ...
        normalized_name="preview traversal",
        encrypted_url="encrypted",
        interval_minutes=15,
        is_active=True,
    )
    ...
    headers = {"X-API-Key": "test-api-key"}
    response = client.get(f"/v1/inputs/{input_row.id}/changes/{change.id}/evidence/after/preview", headers=headers)
    assert response.status_code == 404
    assert response.json()["detail"] == "Evidence file not found"
